...
try {
    ...
    for (let index = 1; index <= db['all']; index++) {
        db[index][1] = htmlToPlainText(db[index][1])
        db[index][2] = htmlToPlainText(db[index][2])
        db[index][3] = htmlToPlainText(db[index][3])
        db[index][4] = htmlToPlainText(db[index][4])

        // changing db
        db[index]['ans'] = htmlToPlainText(db[index]['ans'])
